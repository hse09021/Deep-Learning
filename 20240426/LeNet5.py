...
def missclassified_samples(dataloader, model, classes):
    model.eval()
    ...
    # 잘못 분류된 샘플 개수 출력
    print("\n* Misclassified samples: %d" % len(misclassified_samples))
    '''
    for i in range(len(misclassified_samples)):
        sample = misclassified_samples[np.random.randint(0, len(misclassified_samples))]
        image, predicted_label, true_label = sample
        
        plt.figure(figsize=(5, 5))
        plt.imshow(image.squeeze().cpu().numpy(), cmap='gray')
        plt.axis('off')
        plt.title(f"({i + 1}) Predicted: {classes[predicted_label]}, True: {classes[true_label]}")
        plt.show()'''
...
plt.plot([e.detach().cpu().numpy() for e in error_train], 'black', label='cost function')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
...
missclassified_samples(test_dataloader, model, classes)